...
const CreateTaskModal = ({ isOpen, onClose, onSuccess, task = null, initialHouseId = null }) => {
  ...
  const fetchMembers = async (houseId) => {
    try {
      ...
      const response = await request(`/house-members/house/${houseId}`, {
        auth: true,
      });
      ...
      if (task) {
        setFormData({
          ...
          expiration_date: task.expiration_date
            ? new Date(task.expiration_date).toISOString().split("T")[0]
            : "",
          state: task.state || "pending",
          id_house: task.id_house,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = task ? `/tasks/${task.id_task}` : "/tasks";

      const payload = { ...formData };
      if (!payload.expiration_date) delete payload.expiration_date;
      if (!payload.id_user) payload.id_user = null;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                >
                  ...
                  {houses.map((house) => (
                    <option key={house.id_house} value={house.id_house}>
                      {house.name}
                    </option>
                  ))}
                </select>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                >
                  <option value="">
                    {!formData.id_house ? "Select house first" : "Unassigned"}
                  </option>
                  {members.map((member) => (
                    <option key={member.id_user} value={member.id_user}>
                      {member.name}
                    </option>
                  ))}
                </select>
